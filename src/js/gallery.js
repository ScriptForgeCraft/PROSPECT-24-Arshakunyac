...
(() => {
  ...
  const valuationThumbs = Array.from(
    document.querySelectorAll(".valuation-thumb"),
  );
  ...
  function navigateValuation(index) {
    ...
    zoomValuationIndex =
      (index + valuationThumbs.length) % valuationThumbs.length;
    const largeSrc =
      valuationThumbs[zoomValuationIndex].dataset.large ||
      valuationThumbs[zoomValuationIndex].src;
  ...
  function openZoom(context = "main", startIndex = 0) {
    ...
    if (context === "main") {
      largeSrc =
        thumbs[activeIndex]?.dataset.large ||
        mainImg?.dataset.large ||
        mainImg?.src;
      const counterEl = document.getElementById("zoomCounter");
      if (counterEl) {
  ...
  function handleZoomTouchMove(e) {
    ...
    if (currentScale <= 1 && zoomSwipeTracking) {
      ...
      if (!zoomSwipeAxis) {
        if (
          Math.abs(dragDistX) > ZOOM_SWIPE_LOCK_AXIS_THRESHOLD ||
          Math.abs(dragDistY) > ZOOM_SWIPE_LOCK_AXIS_THRESHOLD
        ) {
  ...
  function handleZoomTouchEnd(e) {
    ...
    if (
      currentScale <= 1 &&
      zoomSwipeAxis === "x" &&
      Math.abs(dragDistX) >= ZOOM_SWIPE_THRESHOLD
    ) {
      if (dragDistX < 0) {
        handleZoomNext();
      } else {
  ...
  zoomOverlay?.addEventListener("pointerup", (e) => {
    ...
    if (dx < TAP_THRESHOLD && dy < TAP_THRESHOLD) {
      ...
      const isButton = target.closest(
        ".zoom-in, .zoom-out, .zoom-reset, .zoom-close, .zoom-arrow",
      );
      const isImage =
        target === zoomImage || target.closest(".swiper-zoom-container img");
  ...
  if (zoomContainer) {
    zoomContainer.addEventListener("wheel", handleZoomWheel, {
      passive: false,
    });
  }
